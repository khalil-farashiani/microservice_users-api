...
func (user *User) Get() *errors.RestErr {
	...
	if getErr := result.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); getErr != nil {
		logger.Error("error when trying to get user bu id", getErr)
		return errors.NewInternalServerError("database error")
	}
	return nil
}
...
func (user *User) Save() *errors.RestErr {
	...
	if saveErr != nil {
		logger.Error("error when trying save user", saveErr)
		return errors.NewInternalServerError("database error")
	}
...
func (user *User) FindByStatus(status string) (Users, *errors.RestErr) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); err != nil {
			logger.Error("error when trying to scan row to get user", err)
			return nil, errors.NewInternalServerError("database error")
		}
